    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @dev Event emitted when a token fee is paid, we don't list all creator addresses because there can be 5
    event TokenForgeFee(
        uint256 creatorFee, address platform, uint256 platformFee, address referrer, uint256 referralFee
    );
    ...
    function mint(address to, uint256 quantity, address referrer) public payable nonReentrant {
        if (quantity == 0) {
        ...
        uint256 referrerFee;
        (creatorFee, platformFee, platformFeeAddr, referrerFee) = _getFees(quantity);

        // If a referrer is passed subtract the referral fee from the total cost
        ...
        uint256 totalFee = creatorFee + platformFee + referrerFee;
        if (msg.value != totalFee) {
            revert IErrors.IncorrectETHAmount(msg.value, totalFee);
        }
        ...
        if (referrerFee > 0) {
            paid += referrerFee;
            (bool payReferrer,) = referrer.call{value: referrerFee}("");
            if (!payReferrer) {
                revert IErrors.FailedToSendEth();
            }
        }
        ...
        if (paid != totalFee) {
            revert IErrors.IncorrectETHAmount(paid, totalFee);
        }

        // Emit a fee event so we know how much was paid
        emit TokenForgeFee(creatorFee, platformFeeAddr, platformFee, referrer, referrerFee);

        // Emit a mint event for analytics.
        emit TokenForgeMint(to, quantity);
    }

    // @dev Allows a user to mint with a comment which is optional
    function mintWithComment(address to, uint256 quantity, string calldata comment, address referrer)
        external
        payable
    {
        mint(to, quantity, referrer);
        if (bytes(comment).length > 0) {
            emit TokenForgeMintComment(to, quantity, comment);
        }
    }

    /// @dev Allows a user to mint directly to the transaction sender.  This will be needed for integrations like farcaster frames.
    function mintToSender(uint256 quantity, string calldata comment, address referrer) external payable {
        mint(msg.sender, quantity, referrer);
        if (bytes(comment).length > 0) {
            emit TokenForgeMintComment(msg.sender, quantity, comment);
        }
    }
    ...
    /// @notice We call the mint factory to determine platform costs.
    function _getFees(uint256 quantity) internal view returns (uint256, uint256, address, uint256) {
        ...
        uint256 referrerFee = mintingFee.referral * quantity;
        return (creatorFee, platformFee, mintingFee.addr, referrerFee);
    }
